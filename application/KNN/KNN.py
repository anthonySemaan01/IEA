...
class KNN(AbstractFeatureExtractor):
    ...
    def get_most_frequent_element(l):
        ...
        # counts how many times an element is present in a list : l [1, 1, 2] --> l.count [2, 2, 1] 1 is present 2
        # times, 1 is present 2 times, 2 is present 1 time
    ...
    def dist(x, y):  # returns sqrt( sum (  (Ai - Bi)^2  ))
        return sum(
            ...
            # zip : x = [1, 2, 3], y = ['a', 'b', 'c'], zip(x, y) = [ [1, 'a'], [2, 'b'], [3, 'c'] ]
            # #here 1 and a are PIXELS THAT WE WILL COMPARE
        )

    def get_training_distances_for_test_sample(X_train, test_sample):
        return [KNN.dist(train_sample, test_sample) for train_sample in
                X_train]  # for every trained digit, we are going to find its distance from the test digit

    # 1 White/Black
    def knn(X_train, y_train, X_test,
            k=3):  # for our point, it looks at ALL OTHER points, calculates distance with them, and takes the point
        # with the smallest DISTANCE    | k : see which one returns best result (better as an odd parameter for no
        # confusion if for example we have 2 classes and a point is confusing, but here we have a lot of classes)
        ...
        for test_sample_idx, test_sample in enumerate(X_test):
            ...
            # print(sorted_distance_indices) # shows : for each test sample, the distance between it and the 100
            # training samples
            # 3)
            candidates: list = [
                (y_train[idx])
