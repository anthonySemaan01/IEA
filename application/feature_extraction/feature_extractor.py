...
'''
    ...
    def get_total_black_pixels(self, path):
        img = cv2.imread(path)
        number_of_black_pix = np.sum(img == 255)  # extracting only black pixels
    ...
    def get_total_left_pixels(self, path):
        img = cv2.imread(path)
        total_left = img[:, :14]
        total_left = np.sum(total_left == 255)
        return total_left

    def get_total_right_pixels(self, path):
        img = cv2.imread(path)
        total_right = img[:, 14:]
        total_right = np.sum(total_right == 255)
        return total_right

    def get_total_up_pixels(self, path):
        img = cv2.imread(path)
        total_up = img[:14, :]
        total_up = np.sum(total_up == 255)
        return total_up

    def get_total_down_pixels(self, path):
        img = cv2.imread(path)
        total_down = img[14:, :]
        total_down = np.sum(total_down == 255)
        return total_down

    def get_sub_pixels1(self, path):
        img = cv2.imread(path)
        sub_area = img[:4, :4]
        sub_area = np.sum(sub_area == 255)
        return sub_area
